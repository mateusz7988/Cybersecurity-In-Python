...
class Particle:
    ...
    def __init__(self, x, y, vx, vy, radius=0.01, styles=None):
        ...
        ADD SPEED VALUES AND ENERGY ON TOP OF THE PARTICLES
        ...
        self.force = (self.mass * np.linalg.norm(self.v)**2)/2
    ...
    def draw(self, ax):
        ...
        ax.add_patch(circle)
        #ax.text(self.x, self.y, f"{self.v}")  #HERE YOU CAN ON/OFF THE SPEED VALUES
...
class Simulation:
    ...
    def __init__(self, n, radius=0.01, styles=None):
        ...
        self.dt = 0.01
        self.particle = self.initialize_particle()
    ...
    @property
    def get_vx(self):
        return self.particle.vx

    @property
    def get_vy(self):
        return self.particle.vy

    @property
    def get_force(self):
        return self.particle.f
    ...
    def handle_boundary_collisions(self, p):
        """Bounce the particles off the walls elastically.  STOPPED HERE """
...
if __name__ == '__main__':
    ...
    y = np.random.random()
    radii = 0.04
    #radii = np.random.random()*0.03+0.02
    styles = {'edgecolor': 'C0', 'linewidth': 2, 'fill': None}
    sim = Simulation(nparticles, radii, styles) 
    sim.do_animation(save=False)
    sim_force = sim.get_force
    speed = (sim.get_vx, sim.get_vy)
    print(f"This is Particle's average Force: ", {sim_force}, "N")
    print(f"This is Particle's average Vx: ", {sim.get_vx}, "m/s")
    print(f"This is Particle's average Vy: ", {sim.get_vy}, "m/s")
    print(f"This is Particle's average speed: ", {np.linalg.norm(speed)}, "m/s")
